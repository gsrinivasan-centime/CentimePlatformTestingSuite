...
def generate_pdf_report(
        ...
        for sub_module in module.get('sub_modules', []):
            ...
            for feature in sub_module.get('features', []):
                ...
                if feature.get('test_cases'):
                    test_data = [["Test ID", "Title", "Type", "Priority", "Status"]]
                    
                    for tc in feature['test_cases']:
                        title_text = tc['title'][:40] + "..." if len(tc['title']) > 40 else tc['title']
                        test_data.append([
                            tc['test_id'],
                            title_text,
                            tc['test_type'],
                    ...
                    test_table = Table(test_data, colWidths=[0.8*inch, 2.5*inch, 0.8*inch, 0.7*inch, 1*inch])
    ...
    if data.get('failed_test_details') and len(data['failed_test_details']) > 0:
        ...
        failed_data = [["Test ID", "Title", "Module", "Sub-Module", "Bug IDs"]]
        for failed_test in data['failed_test_details']:
            failed_data.append([
                ...
                failed_test['module_name'],
                failed_test.get('sub_module', 'N/A'),
                failed_test.get('bug_ids', 'None')
            ])
        
        failed_table = Table(failed_data, colWidths=[0.8*inch, 2*inch, 1.5*inch, 1.2*inch, 1*inch])
